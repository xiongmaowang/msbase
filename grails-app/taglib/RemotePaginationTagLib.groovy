...
class RemotePaginationTagLib {
    ...
    def remotePaginate = {attrs , body->
		...
		//页面上最多显示数字数量 现在是6
        Integer maxsteps = (params.maxsteps ?: (attrs.maxsteps ?: 6))?.toInteger()
        ...
        if (currentstep > firststep) {
            linkTagAttrs.class = 'fenye_dijit_common fenye_dijit_qy'
            linkParams.offset = offset - max 
			previousCache=wrapInListItem(bootstrapEnabled,remoteLink(linkTagAttrs.clone()) {
        ...
        if (steps && laststep > firststep) {
            linkTagAttrs.class = 'fenye_dijit_btn'
            ...
            if (beginstep > firststep) {
                linkParams.offset = 0
                linkTagAttrs.class = "fenye_dijit_common fenye_dijit_sy"
			...
			if(previousCache){
				writer<<previousCache
			}
            // display paginate steps
            (beginstep..endstep).each {i ->
                if (currentstep == i) {
                    String currentStepClass = bootstrapEnabled ? "active" : "currentStep"
                    writer << wrapInListItem(bootstrapEnabled,"<span class=\"fenye_dijit_btn fenye_dijit_current\">${i}</span>")
                } else {
            ...
            if (endstep < laststep) {
				...
				linkTagAttrs.class = "fenye_dijit_common fenye_dijit_wy"
                endstepCache = wrapInListItem(bootstrapEnabled,remoteLink(linkTagAttrs.clone()) { '尾页'})
				linkTagAttrs.class = 'fenye_dijit_btn'
            }
        }
        // display next link when not on laststep
        if (currentstep < laststep) {
            linkTagAttrs.class = 'fenye_dijit_common fenye_dijit_hy'
            linkParams.offset = offset + max
            writer << wrapInListItem(bootstrapEnabled,remoteLink(linkTagAttrs.clone()) {
        ...
        if ((alwaysShowPageSizes || total > max) && pageSizes) {
            ...
            writer << wrapInListItem(bootstrapEnabled,"<span>" + select(from: pageSizes, value: max, name: "max", onchange: "${remoteFunction(linkTagAttrs.clone())}" ,class: 'fenye_dijit_common',
                    optionKey: isPageSizesMap?'key':'', optionValue: isPageSizesMap?'value':'') + "</span>") 
            writer << wrapInListItem(bootstrapEnabled,"<span>转到<input type=\"number\" id=\"pageselecter\" class=\"fenye_dijit_common\"/>页</span>")    
			writer<<"<input type=\'button\' onclick=\'var data =${JsonOutput.toJson(linkParams)}; var topage=parseInt(document.getElementById(\"pageselecter\").value); var max=${max} ;var total=${total}; var laststep=${laststep} ;if(topage){if(topage<=0){data.offset=0}else if(topage>laststep){data.offset=(laststep-1)*max}else{data.offset=(topage-1)*max}}else{data.offset=0}; jQuery.ajax({type:\"POST\",data:data, url:\"${request.getContextPath()}/${linkParams.controller}/${attrs.action}\",success:function(data,textStatus){jQuery(\"#${attrs.update}\").html(data);},error:function(XMLHttpRequest,textStatus,errorThrown){}});return false;\' total=\"${total}\" pagesizes=\"${pageSizes}\" class=\"fenye_dijit_jump_btn\" value=\"跳转\"/>"
